...
import { DepartmentData } from '../models/department-data.model'; 
...
export class DepartmentService extends ResourceService<Department>{
  ...
  constructor(
    ...
    super(http, environment.authApiUrl, "employee/dept", new DepartmentSerializers(), authService); 
  }
  ...
  getDepartmentList(department: Department): Observable<any> {
    ...
    return this.http.get(this.DEPARTMENT_STATUS_LIST, { headers}).pipe(
      map((data: any) => data
      ));
  }
  ...
  getDepartmentParentList(): Observable<any> {
    ...
    return this.http.get(this.DEPARTMENT_PARENT_LIST, { headers}).pipe(
      map((data: any) => data
      ));
  }
  ...
  public deleteDepartment(id: string) {
    ...
    return this.http
      .delete(`${this.DEPARTMENT_DELETE, { headers}}`, {
  ...
  getDepartmentByNo(_id: string) {
    ...
    return this.http.get(this.DEPT_BY_NO, { params: new HttpParams().set('id', _id), headers }).pipe(
  ...
  getOpdDepartmentList(): Observable<any> {
    ...
    return this.http.get(this.OPD_DEP_LIST, { headers}).pipe(
      map((data: any) => data
      ));
  }

  createDepartment(_departmentData: DepartmentData) {
          const token = this.authService.getAccessToken();  // get token from your auth service
        const headers = new HttpHeaders({
            'Authorization': `Bearer ${token}`
        });
    return this.http.post(this.DEPARTMENT_SAVE, {_departmentData, headers}).pipe(
      map((data: any) => data
      ));
  }

  updateDepartment(_departmentData: DepartmentData) {
          const token = this.authService.getAccessToken();  // get token from your auth service
        const headers = new HttpHeaders({
            'Authorization': `Bearer ${token}`
        });
    return this.http.put(this.DEPARTMENT_UPDATE, {_departmentData, headers}).pipe(
      map((data: any) => data
      ));
  }
  ...
  getDepartmentListByUserNo(): Observable<any> {
    ...
    return this.http.get(this.DEPARTMENT_LIST_BY_USER_NO, { headers}).pipe(
      map((data: any) => data
      ));
  }
}