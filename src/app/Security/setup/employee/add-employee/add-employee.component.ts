...
export class AddEmployeeComponent implements OnInit {
  ...
  onSaveOrUpdate(): void {
    ...
    this.personnelData.sinOfficerFlag = this.personnelData.sinOfficerFlag ? 1 : 0;
    ...
    if (this.personnelData.id) {
      this.empService.updateEmployee(this.personnelData).pipe(
        finalize(() => {
          this.isSaving = false;
        })
      ).subscribe({
      ...
      this.empService.saveEmployee(this.personnelData).pipe(
        finalize(() => {
          this.isSaving = false;
        })
      ).subscribe({
        next: (res: { success: boolean; message?: string }) => {
          if (res.success) {
            this.toastr.success(res.message || "Employee saved successfully!");
            this.onClose.next(true);
            this.bsModalRef.hide();
          } else {
        ...
        error: (err) => {
          this.toastr.error("Something went wrong while saving. Please check your network or try again.");
        }
      });
    }
  }
