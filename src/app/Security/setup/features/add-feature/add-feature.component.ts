...
export class AddFeatureComponent implements OnInit {
  ...
  saveFeatures() {
    ...
    if (this.feature.id) {
      this.featureService.updateFeature(this.feature).pipe(
        finalize(() => {
          this.isSaving = false;
        })
      ).subscribe({
        next: (res: { success: boolean; message?: string }) => {
          if (res.success) {
            this.toastr.success(res.message || 'Feature updated successfully!');
        ...
        error: (err) => {
          this.toastr.error('Something went wrong while updating the feature. Please try again.');
          this.onClose.next(false);
        }
      });

    } else {
      this.featureService.saveFeature(this.feature).pipe(
        finalize(() => {
          this.isSaving = false;
        })
      ).subscribe({
        ...
        error: (err) => {
          this.toastr.error('Something went wrong while saving the feature. Please try again.');
          this.onClose.next(false);
        }
      });
    }
  ...
  selectIcon() {
    ...
    this.iconModal.content.selectedIcon.subscribe((res: { iconValue: string; }) => {
  ...
  validateFeatureCode() {
    if (this.feature.id && (this.existingFeature != this.feature.submenuId)) {
      this.featureService.validateFeatureCode(this.feature.submenuId).subscribe((res: { success: any; }) => {
        if (!res.success) {
          this.validate = true;
        }
      })
